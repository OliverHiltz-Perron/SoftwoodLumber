...
# Query configuration
EMBEDDING_PREFIX = "search_document:"  # Prefix for embeddings
SIMILARITY_THRESHOLD = 0.6  # Threshold for similarity
...
def process_propositions_with_embeddings(json_path, prefix=EMBEDDING_PREFIX, threshold=SIMILARITY_THRESHOLD, top_k=TOP_K):
    """
    Process all propositions in the JSON file, find similar propositions, and add them to the JSON
    """
    logging.info("Starting semantic analysis of propositions...")
    print("\nStarting semantic analysis of propositions...")
    ...
    logging.info(f"Total propositions: {len(all_propositions)}")
    print(f"Total propositions: {len(all_propositions)}")
    
    if len(all_propositions) == 0:
        logging.warning("No propositions found in the data. Skipping embedding process.")
        print("No propositions found in the data. Skipping embedding process.")
        return data
    
    # Create embeddings for all propositions
    logging.info("Creating embeddings for all propositions...")
    ...
    logging.info("Finding similar propositions for each proposition...")
    ...
    logging.info(f"Semantic analysis complete. Results saved to {json_path}")
    print(f"Semantic analysis complete. Results saved to {json_path}")
    return data

# ---------- Original Propositions.py Functions ----------
...
def process_file(input_file_path, all_results):
    ...
    if gemini_response:
        ...
        if gemini_response.strip() == "NA":
            ...
            updated_data = save_propositions_json([all_results[0], process_result], append_mode=True)
            if updated_data:
                all_results = updated_data
        ...
        # Save after each file is processed - only sending the new document, not the full array
        updated_data = save_propositions_json([all_results[0], error_result], append_mode=True)
        if updated_data:
            all_results = updated_data
...
def main():
    ...
    logging.info(f"Proposition extraction complete. Success: {success_count}, Errors: {error_count}")
    print(f"\nProposition extraction complete. Successfully processed {success_count} out of {file_count} files.")
    
    # Once all files are processed, run the embedding process on the full dataset
    try:
        print("\nStarting semantic analysis of propositions...")
        logging.info("Running semantic analysis on extracted propositions...")
        
        # Process the propositions with embeddings
        ...
        print(f"Final output with semantic relationships saved to {OUTPUT_FILENAME}")
        
    except Exception as e:
        logging.exception(f"Error during semantic analysis: {str(e)}")
        print(f"\nError during semantic analysis: {str(e)}")
        print("Proposition extraction was completed, but semantic analysis failed.")
