...
class MarkdownProcessor:
    ...
    def process_markdown_file(self, markdown_file, output_file, prompt):
        ...
        try:
            ...
            # Generate and process content using Gemini API
    ...
    def _generate_and_process_content(self, prompt, markdown_content):
        """Generate and process content using the Gemini API"""
        try:
            ...
            if json_match:
                gemini_output = json_match.group(1)
            ...
            try:
                ...
                print("Warning: Gemini output is not valid JSON. Attempting to fix...")
                return self._retry_json_parsing(gemini_output, prompt, markdown_content)
                
        except Exception as gemini_error:
            print(f"Gemini API error: {gemini_error}")
            return self.create_template_json()

    def _retry_json_parsing(self, gemini_output, prompt, markdown_content):
        """Retry JSON parsing with fixes or a second attempt"""
        # Try to fix the JSON
        data = self.fix_json(gemini_output)
        ...
        if data is None:
            ...
            try:
                ...
                retry_response = self.model.generate_content(retry_prompt + markdown_content)
                retry_output = retry_response.text
...
def main():
    ...
    for markdown_file in markdown_files:
        ...
        processor.process_markdown_file(markdown_file, output_file, prompt)
